...
the design fixes each pronoun's referent precisely: `me`/`I`/`you` are always
...
from soe import models, parser, pronouns
...
def court():
    ...
    gs = models.GameState()
    gs.turn_number = 1
    for cid, name in [("c1", "Highfell"), ("c2", "Velika"),
                      ("c3", "Calder"), ("c4", "Redport")]:
...
def test_me_and_you_both_mean_the_leader(court):
    assert rewrite(court, "Have Alan Reed give me 100 gold.") == \
        "have alan reed give billy jones 100 gold"
    assert rewrite(court, "Have Alan Reed give you 100 gold.") == \
        "have alan reed give billy jones 100 gold"


def test_me_means_the_leader_even_when_somebody_else_acts(court):
    # Design: "you should always use the pronoun me or you when referring to
...
def test_me_is_a_whole_word(court):
    ...
    assert rewrite(court, "Have Alan Reed mine silver.") == \
        "have alan reed mine silver"
...
def test_him_is_the_last_named_man_not_the_current_agent(court):
    # the design's own example: him is Mark Bolton, because Donald Nap is the
...
def test_him_never_means_the_leader(court):
    # Design: him refers to Alan Reed, not to Billy Jones, even though the
    # leader was named later in the sentence.
    result = rewrite(
        court,
        "Have Alan Reed give 10 gold to Billy Jones. "
        "Have him go to Highfell.")
    assert result == "have alan reed go to highfell"
...
def test_a_person_inside_a_list_cannot_be_the_referent(court):
    # Design: him cannot be Doctor McCoy, who is linked to the 10 soldiers.
    result = rewrite(
        court,
        "Assign 10 soldiers and Doctor McCoy to Alan Reed. "
        "Have him go to Velika.")
    assert result == "have alan reed go to velika"
...
def test_the_agent_of_an_earlier_order_is_still_a_referent_later(court):
    ...
    result = rewrite(court, "Have Alan Reed tax. Have Mary Wise follow him.")
    assert "alan reed" in result


def test_an_unresolvable_pronoun_is_left_alone(court):
    """Better an honest 'not found' than a silently wrong actor."""
    assert rewrite(court, "Have him go to Velika.") == "have him go to velika"
...
def test_himself_is_not_mangled_by_the_him_rule(court):
    ...
    result = rewrite(court, "Have Alan Reed tax. Have Bill Fenton bless himself.")
...
def test_it_means_a_single_unnamed_thing(court):
    assert rewrite(court, "Recruit 1 worker. Assign it to Alan Reed.") == \
        "assign 1 worker to alan reed"
...
def test_a_mass_noun_takes_it_however_much_there_is(court):
    ...
    assert rewrite(court, "Buy 10 stone. Give it to Alan Reed.") == \
        "give 10 stone to alan reed"


def test_them_can_mean_a_group_of_people(court):
    result = rewrite(
        court,
        "Have Alan Reed and Mary Wise tax for 4 weeks. Assign them to me.")
    assert result == "assign alan reed and mary wise to billy jones"
...
def test_a_later_referent_replaces_an_earlier_one(court):
    ...
    result = rewrite(
        court,
        "Recruit 10 soldiers. Buy 10 horses. Assign them to Alan Reed.")
    assert result == "assign 10 horses to alan reed"
...
def test_it_can_mean_a_magical_item(court):
    ...
    result = rewrite(court, "Charge *Ampu* to 75 power. Give it to Alan Reed.")
    assert result == "give *ampu* to alan reed"
...
def test_a_pronoun_order_parses_into_a_real_order(court):
    orders = parser.parse_orders(
        "Have Alan Reed tax. Have him go to Velika.", court, "p1")
...
def test_teleporting_me_with_a_wand_parses(court):
    ...
    court.magical_items["i1"] = models.MagicalItem(
        id="i1", name="*Doramba*", item_type=ItemType.WAND,
        holder_character_id="m", power_current=60, power_max=75,
        skill_level=80, spell="teleport")
    orders = parser.parse_orders(
        "Have Doctor McCoy teleport me to Redport using *Doramba*.",
        court, "p1")
    ...
    assert orders[0].wand_name == "*doramba*"
...
def test_referents_do_not_leak_between_players(court):
    ...
    parser.parse_orders("Have Alan Reed tax.", court, "p1")
    # p2 never named anybody, so `him` has nothing to bind to.
    orders = parser.parse_orders("Have him go to Velika.", court, "p2")
