    ...
    for player_id, orders in orders_by_player.items():
        for order in orders:
            ...
            if isinstance(order, TeleportOrder):
                ...
                # Design: the power needed "is equal to the total encumbrance
            ...
            # Process Fly
                ...
                # Design: "The magical power needed to fly is one-fifth (1/5)
...
def process_conjure(orders_by_player: Dict[str, List[Order]], game_state: GameState,
    ...
    Design: needs magic skill 25. The spell spends *all* the caster's power,
...
def process_charge(orders_by_player: Dict[str, List[Order]], game_state: GameState,
    ...
    Design: only magic power transfers, never religious power, and the
    ...
    for player_id, orders in orders_by_player.items():
        for order in orders:
            ...
            for target in order.targets:
                ...
                # transfer the design contemplates.
...
def process_magic_free_zones(game_state: GameState, turn_log: TurnLog):
    ...
    Design: in such a place magical power "does not exist, either in people
...
def process_item_upkeep(game_state: GameState, turn_log: TurnLog):
    ...
    A player is notified when a magical item disappears,
