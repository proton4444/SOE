...
from soe import config, fog, groups, order_queue, territory
...
def process_messages(orders_by_player: Dict[str, List[Order]],
    ...
    Design: "A character may give a message to any other character. If they
...
def _message_audience(order: MessageOrder,
    ...
    A prisoner's own player still receives anything sent to them, per the design,
...
def process_post(orders_by_player: Dict[str, List[Order]],
    ...
    Design: only in a town secured by one of your characters, and the poster
...
def process_report(orders_by_player: Dict[str, List[Order]],
    ...
    Design: the report covers the reporter's own status plus what they can
...
def _compose_report(subject: Character, brief: bool, game_state: GameState,
    ...
    The full form follows the shape of the example in the design: the reporter
    ...
    # unnamed units: "Captain Bram Kell (combat 20, magic 25), Adept Carolyn
    # Bond, 39 soldiers, 307 gold, currently awaiting orders in Calder."
...
def process_address_and_password(orders_by_player: Dict[str, List[Order]],
    ...
    the design treats both as taking effect as soon as they are parsed rather
...
def expire_postings(game_state: GameState, turn_log: TurnLog):
    ...
    Design: "A posting will remain in effect until you no longer secure the
