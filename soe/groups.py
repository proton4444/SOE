...
The whole command language is built on groups: when an order goes to a
...
Three rules from the design drive everything here:
...
from soe.models import Character, GameState, LocationPosition, UnitStack
...
def group_members(leader_id: str, game_state: GameState) -> List[Character]:
    ...
    Design: "If a person who is assigned has other people or items assigned
...
def group_soldier_count(leader: Character, game_state: GameState,
    ...
    faction pool until somebody is given them, and the design still treats them
