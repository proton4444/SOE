...
from soe.models import PopulationBand, RoadQuality, UnitType, ShipType
...
# One turn is one game week. the design's default game-to-real-time ratio is 7,
...
# Daily wages for common labour, by population band. the design: work is easy
...
# Design: a trainer needs the appropriate skill at least 10 (combat to
# train soldiers, sailing to train sailors).
TRAIN_MIN_TRAINER_SKILL = 10

# Design: a level 50 trainer trains 5 workers to level 1 in a week, i.e.
...
# Design: each week the computer spends about population/100 gold from the
...
# Design: passage costs the group's total encumbrance in gold. The engine
...
# Design: an independent character accepts an offer of at least half the
...
# Design: an elite unit's level rises about 1 partial point per week, from
...
# Design: salary is the number of soldiers times the combat level per
...
# Magic power costs.
# TELEPORT and FLY are priced by encumbrance, not distance -- the design puts
# no limit on teleport distance -- so see `encumbrance.py` for their costs.
...
# Design: a message is "limited to a maximum of 2500 bytes (about 1 full page
# of dense print)"; a posting at the gates to 256 characters.
MESSAGE_MAX_LENGTH = 2500
POST_MAX_LENGTH = 256

# Design: "A password must contain between 8 and 64 characters". Shorter and
...
# Nothing fixes an item's strength: neither finder nor conjurer has any say
# in how powerful the result turns out. Every item is therefore rolled from
# these ranges when it is found or conjured.
...
# CONJURE: minimum magic skill to attempt the spell, per the design.
CONJURE_MIN_MAGIC_SKILL = 25

# SCAN: an orb spends one power per ten miles to the scanned location
# (the design). When the route carries `distance_miles` from the map the miles
...
def get_hop_cost(road) -> float:
    ...
    With `distance_miles` on the map (the design: travel time depends on the
