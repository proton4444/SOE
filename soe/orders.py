...
class Order(ABC):
    ...
    Attributes:
        ...
        explicit_actor: True when the order named its actor rather than falling
            back to the faction leader -- the HAVE form. the design makes that
    ...
    # "go to Redport and recruit 10 soldiers" is parsed before the move runs,
...
class TeleportOrder(Order):
    ...
    Attributes:
        ...
        target_name: Original target name from text (for reporting)
        wand_name: Wand named with `with`/`using`. the design: a wand is never
...
class AssignOrder(Order):
    ...
    Attributes:
        ...
        item_ids: Magical items being handed over. the design gives items by
            name -- "Give *Sarema* to Alan Reed" -- so a GIVE may name an item
...
class RepeatOrder(Order):
    ...
    or STOP, which is what a bare `repeatedly` means in the design.
    """
...
class JoinOrder(Order):
    ...
    Design: JOIN does the same thing as ASSIGN, but is given to the
...
class SupportOrder(Order):
    ...
    Design: the supporter joins the battle "as if they had given the same
...
class ReportOrder(Order):
    ...
    Design: QUERY does exactly what REPORT does, except that it reaches a
...
class PasswordOrder(Order):
    ...
    Design: between 8 and 64 characters. Anything shorter is replaced by a
...
def create_order_from_type(order_type: str, player_id: str, original_text: str = "") -> Optional[Order]:
    ...
    order_map = {
        ...
        "COME": MoveOrder,  # Design: "COME -- see the GO command"
        "KILL": KillOrder,
        "EXECUTE": KillOrder,
