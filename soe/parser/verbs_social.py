...
def parse_join_order(sentence: str, game_state: GameState, player_id: str) -> Optional[JoinOrder]:
    ...
    "Have Alan Reed join General Bill Hayden" or the bare "join Mike Holmes",
...
def _split_swallowed_command(part: str) -> tuple[str, str]:
    ...
    recipient called `aurelia report`. the design recovers from a parse error by
    ...
    from soe.parser.dispatch import _COMMAND_VERBS
...
def _resolve_recipients(text: str, order, game_state: GameState,
    ...
    the design allows three kinds of addressee, and they are told apart by what
...
def parse_message_order(sentence: str, game_state: GameState,
    ...
    Design: "With SAY, the name of the recipient must follow the preposition
    to which, in turn, follows the message. With TELL, the order is reversed
    and the preposition to is not used."

        Have Alan Reed say "Not on your life!" to King Bodo Bunji.
        Tell everyone "Emperor Bram Kell has declared himself ruler!"
    """
...
def parse_report_order(sentence: str, game_state: GameState,
    ...
    Parse REPORT and QUERY, with the optional `briefly` adverb.
        ...
        Report.
        Query Bill Johnson and Alan Reed.
...
def parse_password_order(sentence: str, game_state: GameState,
    ...
    the design accepts `Password SerendipityDoDah` as well as the quoted form,
