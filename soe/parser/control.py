...
def parse_if_order(sentence: str, game_state: GameState, player_id: str) -> Optional[IfOrder]:
    ...
    # to Calder" is two orders. Branch bodies do NOT inherit the head's
    # HAVE target -- "if he has 1000 soldiers then go to Redport" has the
...
def parse_if_condition(text: str, game_state: GameState, player_id: str) -> Optional[dict]:
    ...
    if unit == "power" and not power_modifier:
        # Design: no modifier means the higher of magic and religion power.
...
def parse_await_order(sentence: str, game_state: GameState, player_id: str) -> Optional[AwaitOrder]:
    ...
    person ("have Mary await Alan Reed"), and a wait to an absolute turn ("wait
    until turn 12"). The design writes the last of these as a calendar date,
...
def parse_halt_order(sentence: str, game_state: GameState, player_id: str):
    ...
    # Both "have Joe halt" and the rules' "immediately stop Alan Reed" name the
