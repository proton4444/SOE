...
# ATTACK takes a character name and nothing else. The battle happens
# where the attacker already stands, and a location is reached with GO in the
# same sentence -- "have him go to <city> and attack <character>". So a trailing
# "in <city>" is not a target qualifier, and folding it into the name sends the
# parser hunting for a character called "Regent Aurelia in Redport": the order
...
def parse_attack_order(sentence: str, game_state: GameState, player_id: str) -> Optional[AttackOrder]:
    ...
    if match:
        ...
        # The GO in "go to Redport and attack" is what puts the attacker there,
...
def parse_capture_order(sentence: str, game_state: GameState, player_id: str) -> Optional[CaptureOrder]:
    ...
    Examples:
        - "Capture Jamu Penda"
        - "Have Alan Reed capture Mary Tarrington and Billy The Kid"
    """
    parser = OrderParserBase(game_state, player_id, sentence)
    order = parser.create_order(CaptureOrder)

    # Pattern: "have <actor> capture <target(s)>"
    # Example: "have Alan Reed capture Mary Tarrington"
...
def parse_free_order(sentence: str, game_state: GameState, player_id: str) -> Optional[FreeOrder]:
    ...
    # Example: "have Alan Reed free Mary"
