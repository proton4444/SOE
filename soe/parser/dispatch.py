...
from soe import config, pronouns
...
# "Have <character> ..." -- the design's form for delegating an order.
HAVE_PREFIX = re.compile(r'^\s*have\s+')

# Adverbs that may sit between `and` and the verb of the next chained command:
# "Buy 10 horses and briefly query <character>", or "and have him and
# <character> ... immediately charge it". They belong to the clause that
...
# command (see the design's THEN sequencing).
...
def _have_target(clause: str) -> str:
    ...
    "bill jenkins" for "have bill jenkins go to ashford", and the whole
    list for "have merlinus and cato vell charge it". A "to" between the
    name and the verb ("have him to go to <city>", which the design allows) is
...
def split_clauses(sentence: str, game_state: GameState,
    ...
    Ashford and attack Mike May" is three commands. A boundary is an `and`
...
def _dispatch_clause(sentence: str, game_state: GameState,
    ...
    # "Buy passage to Redport": the buy branch above already tried and failed
...
def parse_orders(raw_text: str, game_state: GameState, player_id: str) -> list[Order]:
    ...
    # study combat. Have Donald Nap go to Highfell and give him 100 gold."
    ...
    for sentence in sentences:
        ...
        # Jenkins, and have him go to Ashford and attack Mike May" is three.
        ...
        # "have him go to Ashford and tax for 3 weeks, and go to Ennistown
