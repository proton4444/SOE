...
the design Appendix B gives every item an encumbrance, and several rules are
priced directly off the total for a group:

    FLY         power = 1/5 of the group's encumbrance, rounded up  (the design)
...
# Encumbrance per single unit, by item kind.
...
def group_encumbrance(leader: Character, game_state: GameState) -> float:
    ...
    This is the airborne/teleport reading of encumbrance. the design: summoned
...
def fly_power_cost(leader: Character, game_state: GameState) -> int:
    """Design: "one-fifth (1/5) of the total encumbrance of the group (rounded up)"."""
...
def teleport_power_cost(target: Character, game_state: GameState) -> int:
    """
    Design: "equal to the total encumbrance of the group (rounded up)".
