...
the design gives items their own economy that cuts across several subsystems,
...
# Item names are polysyllabic and vaguely Polynesian. These syllables
# generate names in that register.
...
def normalize_item_name(name: str) -> str:
    ...
    Players may or may not type the asterisks, so both `*Sarema*` and `Sarema`
...
def can_reach_item(character: Character, item: MagicalItem,
    ...
    Design: the item need not be in his possession, as long as whoever holds
...
def spend_magic_power(character: Character, amount: int,
    ...
    Design: "The power in a crystal is always tapped before the natural power
...
# Design: "An amulet NEVER provides skill in magic or religion."
...
def amulet_skill_level(character: Character, skill: str,
    ...
    Design: "If a person possesses more than one amulet for the same skill,
...
def ring_protection(character: Character, game_state: GameState,
    ...
    Design: "If a person possesses more than one ring, then the most powerful
...
def apply_ring_protection(hit_chance: float, protection: int) -> float:
    ...
    The odds are worked in whole percent: a 74% chance against a prot-3 ring
...
# the design lists the words a player may use to describe a wand's spell. They
...
def find_wand(character: Character, name: str, spell: str,
    ...
    Design: "A wand will never be used automatically - its name must be
...
def make_item(game_state: GameState, rng: random.Random, item_type: ItemType,
    ...
    the design is explicit that the finder or conjurer gets no say in how
...
def describe(item: MagicalItem, game_state: Optional[GameState] = None) -> str:
    """
    Format an item the way the design shows it on a status report.

    e.g. `*Sarema* [amulet, trading 72]`, `*Velika* [crystal, power 51/60]`,
    `*Doramba* [wand, teleport 62/75, 3d]`. The trailing day count appears
...
def regenerate(game_state: GameState) -> None:
    ...
    Orbs and wands regain one point per day automatically. A crystal
...
def drain_magic_free_zone(character: Character, game_state: GameState) -> bool:
    ...
    Design: in such a location magical power does not exist "either in people
