...
`rules.md` describes an asynchronous game: orders are placed on a queue and
...
it still resolves in the turn it was given -- which is exactly what `rules.md`
...
def _enqueue_for_actor(actor_id: str, actor_orders: List[Order],
    ...
    Everything after a REPEAT is its body, per `rules.md`, where `repeatedly`
