...
`rules.md` ties visibility to position (inside / outside / near a city) and to
...
# rules.md: people near a location have "very slim" chance of being seen by
# other people also near the same location.
_NEAR_VS_NEAR_FACTOR = 0.10

# rules.md: "the chance of detection of a lurking individual or group will be
...
def parse_position_prefix(text: str) -> Tuple[LocationPosition, str]:
    ...
    "outside Madegi Doy" -> (OUTSIDE, "Madegi Doy")
    "Riverton"           -> (INSIDE, "Riverton")
    """
...
def effective_skill_level(character: Character) -> float:
    """
    rules.md: effective skill = sqrt(sum of squares of all skill levels).
